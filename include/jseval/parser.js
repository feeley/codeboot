...
function Parser(scanner, warnings)
{
    ...
    this.atable  = Grammar.action_table;
    this.gtable  = Grammar.goto_table;
    this.rtable  = Grammar.reduction_table;
...
Parser.prototype.syntax_error = function (loc, msg)
{
    this.scanner.syntax_error(loc, msg);
};


// method syntax_warning(loc, msg)

Parser.prototype.syntax_warning = function (loc, msg)
{
    this.scanner.syntax_warning(loc, msg);
};
...
Parser.prototype.push = function (delta, new_category, value)
{
    ...
    //    print(value.loc.toString() + ": type: " + value.type);
};
...
function prefix_op1(p, op, expr)
{
    var oper = prefix_op1_table[op.cat];
    check_lvalue(p, oper, expr);
...
function postfix_op1(p, expr, op)
{
    var oper = postfix_op1_table[op.cat];
    check_lvalue(p, oper, expr);
...
function op2(p, expr1, op, expr2)
{
    var oper = op2_table[op.cat];
    check_lvalue(p, oper, expr1);
...
function check_lvalue(p, op, expr)
{
    if ((is_assign_op1(op) || is_assign_op2(op)) && !is_lvalue(expr))
        p.syntax_error(expr.loc,
                       "invalid assignment destination");
}

function is_ref(ast)
{
    return ast instanceof Ref;
}

function is_prop_access(ast)
{
    return ast instanceof OpExpr && is_prop_access_op(ast.op);
}
...
{
    return is_ref(ast) || is_prop_access(ast);
}
...
{
    // never called?
    // print(arg1.loc.toString() + ": arg1");
    // print(arg2.loc.toString() + ": arg2");
    return arg1;
}
...
{
    if (p.warnings.non_integer === true)
    {
        if (!bignum_instance(NUMBER.value) &&
            Math.floor(NUMBER.value) !== NUMBER.value)
        {
            p.syntax_warning(NUMBER.loc,
...
{
    return postfix_op1(p, LeftHandSideExpr, PLUSPLUS);
}

function PostfixExpr_3(p, LeftHandSideExpr, MINUSMINUS)
{
    return postfix_op1(p, LeftHandSideExpr, MINUSMINUS);
}
...
{
    return op2(p, MultiplicativeExpr, MULT, UnaryExpr);
}

function MultiplicativeExpr_3(p, MultiplicativeExpr, DIV, UnaryExpr)
{
    if (p.warnings.division === true)
        p.syntax_warning(DIV.loc,
                         "use of '/' operator");

    return op2(p, MultiplicativeExpr, DIV, UnaryExpr);
}

function MultiplicativeExpr_4(p, MultiplicativeExpr, MOD, UnaryExpr)
{
    return op2(p, MultiplicativeExpr, MOD, UnaryExpr);
}
...
{
    return op2(p, AdditiveExpr, PLUS, MultiplicativeExpr);
}

function AdditiveExpr_3(p, AdditiveExpr, MINUS, MultiplicativeExpr)
{
    return op2(p, AdditiveExpr, MINUS, MultiplicativeExpr);
}
...
{
    return op2(p, ShiftExpr, LSHIFT, AdditiveExpr);
}

function ShiftExpr_3(p, ShiftExpr, RSHIFT, AdditiveExpr)
{
    return op2(p, ShiftExpr, RSHIFT, AdditiveExpr);
}

function ShiftExpr_4(p, ShiftExpr, URSHIFT, AdditiveExpr)
{
    return op2(p, ShiftExpr, URSHIFT, AdditiveExpr);
}
...
{
    if (p.warnings.equality === true)
        p.syntax_warning(EQEQ.loc,
                         "use of '==' operator");

    return op2(p, EqualityExpr, EQEQ, RelationalExpr);
}

function EqualityExpr_3(p, EqualityExpr, NE, RelationalExpr)
{
    if (p.warnings.equality === true)
        p.syntax_warning(NE.loc,
                         "use of '!=' operator");

    return op2(p, EqualityExpr, NE, RelationalExpr);
}

function EqualityExpr_4(p, EqualityExpr, STREQ, RelationalExpr)
{
    return op2(p, EqualityExpr, STREQ, RelationalExpr);
}

function EqualityExpr_5(p, EqualityExpr, STRNEQ, RelationalExpr)
{
    return op2(p, EqualityExpr, STRNEQ, RelationalExpr);
}
...
{
    return op2(p, EqualityExprNoIn, EQEQ, RelationalExprNoIn);
}

function EqualityExprNoIn_3(p, EqualityExprNoIn, NE, RelationalExprNoIn)
{
    return op2(p, EqualityExprNoIn, NE, RelationalExprNoIn);
}

function EqualityExprNoIn_4(p, EqualityExprNoIn, STREQ, RelationalExprNoIn)
{
    return op2(p, EqualityExprNoIn, STREQ, RelationalExprNoIn);
}

function EqualityExprNoIn_5(p, EqualityExprNoIn, STRNEQ, RelationalExprNoIn)
{
    return op2(p, EqualityExprNoIn, STRNEQ, RelationalExprNoIn);
}
...
{
    return op2(p, BitwiseANDExpr, BITAND, EqualityExpr);
}
...
{
    return op2(p, BitwiseANDExprNoIn, BITAND, EqualityExprNoIn);
}
...
{
    return op2(p, BitwiseXORExpr, BITXOR, BitwiseANDExpr);
}
...
{
    return op2(p, BitwiseXORExprNoIn, BITXOR, BitwiseANDExprNoIn);
}
...
{
    return op2(p, BitwiseORExpr, BITOR, BitwiseXORExpr);
}
...
{
    return op2(p, BitwiseORExprNoIn, BITOR, BitwiseXORExprNoIn);
}
...
{
    return op2(p, LogicalANDExpr, AND, BitwiseORExpr);
}
...
{
    return op2(p, LogicalANDExprNoIn, AND, BitwiseORExprNoIn);
}
...
{
    return op2(p, LogicalORExpr, OR, LogicalANDExpr);
}
...
{
    return op2(p, LogicalORExprNoIn, OR, LogicalANDExprNoIn);
}
...
{
    return op2(p, LeftHandSideExpr, AssignmentOperator, AssignmentExpr);
}
...
{
    return op2(p, LeftHandSideExpr, AssignmentOperator, AssignmentExprNoIn);
}
...
{
    return op2(p, Expr, COMMA, AssignmentExpr);
}
...
{
    return op2(p, ExprNoIn, COMMA, AssignmentExprNoIn);
}
