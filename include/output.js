...
(function (exports) {
    ...
    PixelGrid.prototype.pageToRelative = function (coord) {
        var rect = getCoords(this.container);
        var w = this.pixelSize + this.borderWidth;
        var x = Math.max(0,
                         Math.min(this.cols-1,
                                  Math.floor((coord.x - rect.x)/w)));
        var y = Math.max(0,
                         Math.min(this.rows-1,
                                  Math.floor((coord.y - rect.y)/w)));
        return { x: x, y: y };
    };
